...
import ModalNewDocumentA from "../../Modal/ModalNewDocumentA";

const TableDocumentA = () => {
  ...
  const getDocuments = async (page) => {
    ...
    await api
      .get(`/list-documentA?page=${page}`)

      .then((response) => {
        setUrl(response.data.url);
        setData(
          response.data.documentA.sort((a, b) => a.name.localeCompare(b.name))
        );
    ...
    <div>
      ...
      <div className="container-table">
        ...
        <div className="buttonPagination">
          {page !== 1 ? (
            <button type="button" onClick={() => getDocuments(1)}>
              Primeira
            </button>
          ) : (
          ...
          {page !== 1 ? (
            <button type="button" onClick={() => getDocuments(page - 1)}>
              {page - 1}
            </button>
          ) : (
            ""
          ...
          {page !== lastPage ? (
            <button type="button" onClick={() => getDocuments(lastPage)}>
              Última
            </button>
          ) : (
            <button type="button" disabled>
